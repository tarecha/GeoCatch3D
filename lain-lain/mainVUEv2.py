...
def run_analysis():
    ...
    try:
        ...
        #optimasi kode biar lebih cepat tidak loop bersarang
...
if __name__ == "__main__":
    ...
    if daftar_ip and ip_lokal != '127.0.0.1':
        ...
        for ip in daftar_ip:
            if cfg.hostportv2 == 80:
                print(f" -> http://{ip}")
                threading.Timer(1.5, buka_browser, args=(ip,)).start()
            else:
                print(f" -> http://{ip}:{cfg.hostportv2}")
                threading.Timer(1.5, buka_browser, args=(ip, cfg.hostportv2)).start()
        ...
        if cfg.hostportv2 == 80:
            ...
            print(f" -> http://127.0.0.1:{cfg.hostportv2}")
            threading.Timer(1.5, buka_browser, args=('127.0.0.1', cfg.hostportv2)).start()
    # Jalankan server
    server.start(host="0.0.0.0", port=cfg.hostportv2, argv=[])