...
if interpolasi == '1':  # Pemilihan interpolasi
    ...
    matrikKecil = interpolasiLinier.interpolasiLinier(matrikKecil)
    matrikOut = interpolasiLinier.interpolasiLinier(matrikOut) # Asumsi interpolasiLinier adalah modul/fungsi yang diimport
...
print(f"[DEBUG main3] Shape: {matrikOut.shape}, dtype: {matrikOut.dtype}")
fileHandler.eksporTIF(matrikOut, latitude, longitude, pixelKolomAwalKoordinat, pixelBarisAwalKoordinat,  cfg.default_crs)
...
koordinatCekungan, matrikFA = analisis.importFlowAccumulation()
matrikFA = np.flipud(matrikFA)
plotter.add_mesh(grid.copy(), scalars=np.rot90(matrikFA, k=-1) , cmap='Accent', opacity=1)
#plotter.add_mesh(grid, scalars=matrikKecil.flatten(), cmap=custom_terrain, show_edges=False,pickable=True, show_scalar_bar=False)

#plotter.add_mesh(grid.copy(), scalars=np.rot90(analisis.importFlowDirection(), k=-1) , cmap='Blues', opacity=1)
