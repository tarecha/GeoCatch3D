...
def interpolasiLinier(matrikInput):
    ...
    # 1. Salin nilai asli (ke baris dan kolom genap)
    matrikOutput[0::2, 0::2] = matrikInput

    # 2. Interpolasi vertikal (baris ganjil, kolom genap)
    matrikOutput[1::2, 0::2] = (matrikInput[:-1, :] + matrikInput[1:, :]) / 2

    # 3. Interpolasi horizontal (baris genap, kolom ganjil)
    matrikOutput[0::2, 1::2] = (matrikInput[:, :-1] + matrikInput[:, 1:]) / 2

    # 4. Interpolasi diagonal (baris ganjil, kolom ganjil)
    matrikOutput[1::2, 1::2] = (matrikInput[:-1, :-1] + matrikInput[1:, :-1] + matrikInput[:-1, 1:] + matrikInput[1:,1:]) / 4
    return matrikOutput